    ...
    fun <T> load(
        ...
        if (activeResource != null) {
            if (callback != null) {
                ...
                callback.onResourceReady(activeResource, null, false)
            } else {
                // 否则直接通知target
                target.onResourceReady(activeResource as T)
                listener?.onResourceReady(activeResource as T, null, target, false)
            }
        ...
        if (cachedResource != null) {
            ...
            if (callback != null) {
                // 如果有callback，通过callback通知
                callback.onResourceReady(cachedResource, null, false)
            } else {
                // 否则直接通知target
                target.onResourceReady(cachedResource as T)
                listener?.onResourceReady(cachedResource as T, null, target, false)
            }
...
class LoadStatus(
    ...
    enum class Status {
        COMPLETED_FROM_ACTIVE,    // 从活跃资源完成
        COMPLETED_FROM_CACHE,     // 从内存缓存完成
        RUNNING,                  // 正在运行
        CANCELLED,                // 已取消
        FAILED                    // 失败
    }
    ...
    fun isCompleted(): Boolean {
        return status == Status.COMPLETED_FROM_ACTIVE || 
               status == Status.COMPLETED_FROM_CACHE
    }
