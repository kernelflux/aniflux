...
class PAGAnimationLoader : AnimationLoader<PAGFile> {
    ...
    override fun loadFromPath(context: Context,path: String): PAGFile? {
        return try {
            ...
            if (file.exists()) {
                ...
                // If file doesn't exist, try direct load (may be network URL or other path)
                PAGFile.Load(path)
            }
        } catch (e: Exception) {
    ...
    override fun loadFromFile(context: Context,file: File): PAGFile? {
        return try {
            ...
            // This causes multiple PAGViews using the same PAGFile to only show animation in the last one
