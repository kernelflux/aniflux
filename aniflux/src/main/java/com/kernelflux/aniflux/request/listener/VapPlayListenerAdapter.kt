...
class VapPlayListenerAdapter(
    ...
    override fun createAnimatorListener(loopCount: Int?): IAnimListener {
        ...
        // Conversion relationship: repeatCount(3) → setLoop(2) → total play 3 times → should have 2 onAnimationRepeat
        ...
        return object : IAnimListener {
            ...
            override fun onVideoRender(
                ...
                // 2. When frameIndex goes from non-0 (especially from last frame) back to 0, indicates new loop started
                ...
                // ✅ Detect loop: frameIndex == 0 and sawLastFrame == true and not first frame
                // If sawLastFrame, means we've completed a full playback, frameIndex == 0 is definitely a loop
                // Or, if lastFrameIndex > 0 and frameIndex == 0, also consider it a loop (from non-first frame back to first frame)
                ...
                if (isLoopDetected) {
                    // Start new loop (from non-first frame back to first frame)
                    notifyAnimationRepeat()
                    // Reset flag, prepare for new loop
                    sawLastFrame = false
                }
            ...
            override fun onVideoComplete() {
                ...
                // In VAPViewTarget, animView.retainLastFrame has been set
            ...
            override fun onVideoDestroy() {
                ...
                // VAP library will automatically call onVideoDestroy for resource cleanup after playback ends (reference HardDecoder.kt:393)
                // This is normal resource cleanup, should not trigger onAnimationCancel()
                // Note: If operations are needed during resource cleanup, handle here, but should not call notifyAnimationCancel()
            }
