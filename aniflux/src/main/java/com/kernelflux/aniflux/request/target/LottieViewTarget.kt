 ...
 * @date: 2025/01/XX
 */
class LottieViewTarget(view: LottieAnimationView) : CustomViewAnimationTarget<LottieAnimationView, LottieDrawable>(view) {
    ...
    override fun onResourceReady(resource: LottieDrawable) {
        ...
        view.apply {
            ...
            // 根据配置设置循环次数
            // LottieAnimationView.repeatCount: LottieDrawable.INFINITE(-1)表示无限循环，0表示不循环，>0表示循环次数
            this.repeatCount = when {
                repeatCount < 0 -> LottieDrawable.INFINITE // -1 表示无限循环
