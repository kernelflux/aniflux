...
object AnimationTypeDetector {
    ...
    /**
     * Animation resource type enumeration
     */
    ...
    fun detectFromBytes(bytes: ByteArray, length: Int): AnimationType {
        ...
        // Detect SVGA file header (SVGA files usually start with a specific byte sequence)
        if (length >= 8) {
            // SVGA files usually start with a specific magic number
        ...
        // Detect PAG file header (PAG files usually start with a specific byte sequence)
        if (length >= 8) {
            // PAG files usually start with a specific magic number
