...
import com.kernelflux.pag.PAGImageViewHelper.DecoderInfo;
...
public class PAGImageView extends View implements PAGAnimator.Listener {
    ...
    //添加资源切换标志
    private final AtomicBoolean isSwitchingResource = new AtomicBoolean(false);
     ...
     */
    public void play() {
        // 关键修复：如果正在切换资源或 duration 为 0，延迟启动
        // 避免竞态条件：切换过程中调用 play() 时，duration 可能还是 0
        if (isSwitchingResource.get() || animator.duration() <= 0) {
            post(new Runnable() {
                @Override
                public void run() {
                    // 延迟后再次检查并启动
    ...
    private void refreshResource(String path, PAGComposition composition, float maxFrameRate) {
        // 防止并发切换
        if (!isSwitchingResource.compareAndSet(false, true)) {
            return;
        }

        try {
            // 关键修复：切换动画前先释放 GL 资源
            // 显式释放 decoder，这会释放相关的 GL Context
            if (decoderInfo.isValid()) {
                try {
            ...
            if (!hasSize()) {
                ...
                isSwitchingResource.set(false);
                return;
            }

            // 同步初始化 decoder（关键修复）
            boolean decoderInitialized = false;
            synchronized (decoderInfo) {
                if (_composition == null && _pagFilePath != null) {
                    _composition = getCompositionFromPath(_pagFilePath);
                }
                if (_composition != null) {
                    decoderInitialized = decoderInfo.initDecoder(
                            _composition, width, height, _maxFrameRate);
                    if (decoderInitialized) {
                        if (_pagFilePath != null) {
                            _composition = null;
                        }
                        refreshMatrixFromScaleMode();
                        freezeDraw.set(false);  // 解除冻结
                    }
                }
            }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 如果资源已设置，立即初始化 decoder（关键修复）
        if (hasSize() && (_composition != null || _pagFilePath != null)) {
            initDecoderInfo();
        }
    ...
    protected void onAttachedToWindow() {
        ...
        // 关键修复：确保 animator 存在
        // 在 RecyclerView 中，View 可能被复用，但 animator 应该一直存在
        // 如果 animator 为 null（不应该发生），重新创建
        if (animator == null) {
            animator = PAGAnimator.MakeFrom(getContext(), this);
        }
        // 注意：我们不在 onDetachedFromWindow 中调用 release()，所以 animator 不会被释放
        // 如果业务层显式调用了 release()，animator 的 nativeContext 会被清空
        // 但这种情况应该由业务层负责重新创建，这里不做处理
    ...
    protected void onDetachedFromWindow() {
        ...
        // 关键修复：只停止动画，不释放资源
        // 在 RecyclerView 中，View 可能被复用，不应该释放 animator
        // 只停止动画，保留 animator 对象以便复用
        if (animator != null) {
            try {
                // 只取消动画，不释放资源
                // release() 会清理 native 资源，导致 View 复用时无法使用
    ...
    protected void finalize() throws Throwable {
        // 关键修复：在 finalize 中显式释放资源
        // 这是 View 真正被销毁时的最后机会，此时可以安全释放资源
        // 注意：finalize 在 GC 线程中执行，但我们已经修复了 nativeFinalize 的死锁问题
        if (animator != null) {
            try {
                // 先取消动画
                if (animator.isRunning()) {
    ...
    public void onAnimationUpdate(PAGAnimator animator) {
        ...
        // 如果正在切换资源，跳过本次更新（关键修复）
        if (isSwitchingResource.get()) {
            return;
        }
        ...
        if (isVisible && (_composition != null)) {
            long newDuration = _composition.duration();
            long currentDuration = animator.duration();
            if (currentDuration != newDuration) {
                animator.setDuration(newDuration);
            }
        } else if (!isVisible && animator.duration() > 0) {
            animator.setDuration(0);
        }
        flush();
        ArrayList<PAGImageView.PAGImageViewListener> arrayList;
