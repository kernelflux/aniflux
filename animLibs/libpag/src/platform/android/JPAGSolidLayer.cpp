...
PAG_API void Java_com_kernelflux_pag_PAGSolidLayer_nativeInit(JNIEnv* env, jclass clazz) {
  PAGSolidLayer_nativeContext = env->GetFieldID(clazz, "nativeContext", "J");
}

PAG_API jint Java_com_kernelflux_pag_PAGSolidLayer_solidColor(JNIEnv* env, jobject thiz) {
...
PAG_API void Java_com_kernelflux_pag_PAGSolidLayer_setSolidColor(JNIEnv* env, jobject thiz, jint color) {
  auto pagLayer = GetPAGSolidLayer(env, thiz);
  if (pagLayer == nullptr) {
    return;
  }
