package com.kernelflux.svgaplayer.drawer
...
import com.kernelflux.svgaplayer.SVGASoundManager
import com.kernelflux.svgaplayer.SVGAVideoEntity
import com.kernelflux.svgaplayer.entities.SVGAVideoShapeEntity
...
internal class SVGACanvasDrawer(
    ...
    private val drawable: SVGADrawable? = null
            ...
            if (audio.startFrame == frameIndex) {
                ...
                if (SVGASoundManager.isInit()) {
                    audio.soundID?.let { soundID ->
                        playId = SVGASoundManager.play(soundID)
                        audio.playID = playId
                    }
                } else {
                    this.videoItem.soundPool?.let { soundPool ->
                        audio.soundID?.let { soundID ->
                            playId = soundPool.play(soundID, 1.0f, 1.0f, 1, 0, 1.0f)
                            audio.playID = playId
                        }
                    }
                }
                // 记录 playID 到 drawable，防止丢失
                playId?.let { drawable?.recordPlayId(it) }
            }
            if (audio.endFrame <= frameIndex) {
                audio.playID?.let { playId ->
                    if (SVGASoundManager.isInit()) {
                        ...
                        this.videoItem.soundPool?.stop(playId)
                    }
                    // 从 drawable 中移除 playID
                    drawable?.removePlayId(playId)
                }
                audio.playID = null
            }
        }
    ...
    @SuppressLint("UseKtx", "DiscouragedPrivateApi")
    ...
    class ShareValues {
        ...
        fun shareMatteCanvas(width: Int, height: Int): Canvas {
            var tmpCanvas = shareMatteCanvas
            if (tmpCanvas == null || width != lastSharedWidth || height != lastSharedHeight) {
                val createBmp = createBitmap(width, height, Bitmap.Config.ALPHA_8)
                tmpCanvas = Canvas(createBmp)
                shareMatteCanvas = tmpCanvas
                sharedMatteBitmap = createBmp
    ...
    class PathCache {
        ...
        fun buildPath(shape: SVGAVideoShapeEntity): Path {
            if (!this.cache.containsKey(shape)) {
                val path = Path()
                shape.shapePath?.apply { path.set(this) }
